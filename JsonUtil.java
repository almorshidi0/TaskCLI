...
/**
 * This class contains utility methods for working with JSON files. The JSON file is assumed to
 * contain an array of JSON objects. The methods in this class can be used to add, read, update, and
 * delete JSON objects from the file. The methods are static and can be used without creating an
...
public class JsonUtil {
  /**
   * Initialize a JSON file to store an array of JSON objects. If the file does not exist, it will
   * be created. If the file already exists, it will not be changed.
   *
  ...
  /**
   * Reads a JSON file containing an array of JSON objects and returns the objects as an array of
   * strings. If the file does not exist, an exception is thrown. If the file exists but is empty,
  ...
  public static String[] readJsonFileAsObjects(String jsonFileName) throws Exception {
    ...
    fileScanner.close();
    String[] objects =
        content.toString().substring(1, content.length() - 1).trim().split("(?<=\\}),(?=\\{)");
    if (objects.length == 1 && objects[0].trim().equals("")) {
      return new String[0];
    }
    for (int i = 0; i < objects.length; i++) objects[i] = objects[i].replace("},", "}");
    return objects;
  }

  /**
   * Adds a JSON object to a JSON file that stores an array of JSON objects. If the file is empty,
   * the JSON object is added directly. If the file already contains JSON objects, the new object is
   * appended with a comma separator.
   *
   * @param jsonFileName the name of the JSON file
  ...
  public static void updateJsonObject(
      String jsonFileName, String oldJsonObject, String newJsonObject) throws Exception {
    File file = new File(jsonFileName);
    StringBuilder content = new StringBuilder();
    try (Scanner fileScanner = new Scanner(file)) {
      while (fileScanner.hasNextLine()) {
